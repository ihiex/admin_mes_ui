              ...
              <el-dropdown>
                ...
                <el-dropdown-menu slot="dropdown">
                  ...
                  <el-input v-model="capacityTarget" type="number" style="width: 80px"
                    @input="handleTargetInput"></el-input>
                </el-dropdown-menu>
              </el-dropdown>
            </div>
          </div>
          <div class="left_aside_bottom">
            <div id="PieChart" style="height: inherit; width: inherit"></div>
          </div>
        </el-aside>
        <el-main class="main">
          <div class="main_top">
            <div>
              ...
              <el-select v-model="params.date" size="mini" placeholder="请选择" @change="handleDateChange">
                <el-option v-for="item in dateArray" :key="item" :label="item" :value="item">
                </el-option>
              </el-select>

              <el-select v-model="params.line" size="mini" placeholder="请选择" @change="handleSelectLine">
          ...
          <div id="LineAndBarChart" class="main_middle">
            <DefectByColor ref="DefectByColor" style="height: 100%" cid="DefectByColor2024" />
            <el-dropdown>
              <span class="el-dropdown-link">
                <i class="el-icon-arrow-down el-icon-setting"></i>
              </span>
          ...
          <!-- <div class="main_bottom">
            <div id="BottomBarChart">
            </div>
            <div class="select_container">
              <el-select v-model="params.selectVal" size="mini" placeholder="Select" style="width: 100px;"
            ...
            <div style="height: 20px; width: 20px; position: absolute; top: 5px; right: 0; text-align: center">
              <el-dropdown>
                ...
                <el-dropdown-menu slot="dropdown">
                  ...
                  <el-input v-model="yieldTarget" type="number" style="width: 80px"
                    @input="handleYieldInput"></el-input>
                </el-dropdown-menu>
              </el-dropdown>
            </div>
          </div>
          <div class="right_aside_bottom" style="padding-top: 3px;">
            <el-table :data="tableData" size="mini" :border="false" style="width: 105%; background-color: transparent;"
              height="98%" row-class-name="row_class_name" header-row-class-name="row_class_name"
              header-cell-class-name="row_class_name" @mouseenter.native="autoScroll(true)"
...
import { getShift, getUserShift, getTimeChangeTimeRangeNew } from '@/utils'
...
export default {
  ...
  methods: {
    ...
    getDefectYield(itemInfo, date) {
      ...
      GetDefectYield(params, '', true).then((res) => {
        ...
        if (res.Success && data) {
          ...
          this.setPieChart("PieChart", data[6])
      ...
      GetUPH(params, true).then((res) => {
        try {
          ...
          if (res.Success) {
            ...
            this.handleTargetInput(this.capacityTarget);
            this.handleYieldInput(this.yieldTarget);
            // 最终良率数据 结束

            // daily
            this.getDailyDataV2(this.tableData)
    ...
    handleSelectChange(val) {
      this.setBottomBarChart("BottomBarChart", 'Tearing @ Textile', val)
    },
    setBarChart(id, data) {
    ...
    setPieChart(id, data) {
      ...
      let option = {
        ...
        tooltip: {
          ...
          formatter: (val) => {
            // console.log(val)
            return `${val.marker} ${val.name}: ${val.data.qty} (${val.value >= 0 ? val.value : '--'}%)`
          }//'{b}: {c} ({d}%)'
        },
        legend: {
          data: legendData, /* [
