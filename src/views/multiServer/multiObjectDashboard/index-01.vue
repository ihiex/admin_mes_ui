      ...
      <el-popover v-model="visible" style="position: absolute; left: 15px; top:0px;z-index: 101;" width="900"
        ...
        <div>
          ...
          <div style="text-align: right; padding-top: 10px;">
            ...
            <el-button type="primary" @click="handleServerSwitch(true)">{{ $t('query') }}</el-button>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    async handleObjChange(val) {
      if (!val) return
      // console.log(this.switchData[val])
      ...
      this.handleDefectChart(this.switchData[val]?.defect, this.dyId, this.switchData[val]?.yieldCumActual, this.yieldCumTarget)
    ...
    handleCumTargetInput(val) {
      ...
      this.handleDefectChart(this.switchData[this.objKey]?.defect, this.dyId, this.switchData[this.objKey]?.yieldCumActual, this.yieldCumTarget);
    },
    ...
    getData(api, isShow) {
      ...
      GetUPH(params, isShow, api).then((res) => {
        ...
        if (res.Success && data) {
          ...
          if (this.oneEvent) {
            this.oneEvent = false;
          } else if (key === this.objKey) {
            // console.log(key)
            // console.log(this.switchData[key]['output'])
            // let _this = this
            // setTimeout(function(){
            // _this.dataFormat(_this.switchData[key]['output'], key)
            // _this.handleObjChange(key)
            // })
            this.setData(key)
          }
        }
    ...
    getDefectYield(api, isShow) {
      ...
      GetDefectYield(params, api, isShow).then((res) => {
        ...
        if (res.Success && data) {
          ...
          this.$set(this.switchData[key], 'defect', list)
        }
    ...
    dataFormat(obj, key) {
      ...
      Object.keys(obj).forEach(k => {
        ...
        // this.processYield[k] = Object.assign({}, o)
      ...
      Object.keys(this.carouselData[key]).forEach(kk => {
        ...
        Object.keys(this.carouselData[key][kk]).forEach(k => {
          let arr = [k]
          arr.push(this.carouselData[key][kk][k].Output)
          arr.push(this.carouselData[key][kk][k].Input - this.carouselData[key][kk][k].Output)
      ...
      Object.values(this.carouselData[key]).forEach(value => {
      ...
      let yca = (sumOutput / sumInput * 100).toFixed(2)
      this.$set(this.switchData[key], 'yieldCumActual', isNaN(yca) ? '--' : yca)
      this.setData(this.titles[this.index])
    },
