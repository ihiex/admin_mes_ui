<template>
  <div v-loading="oneEvent" class="main_multiserver">
    <div class="multiserver_title">
      ProjectName: {{ title[devServer[serverIndex - 1]] }}
    </div>
    <el-carousel
      id="MultiserverDashborad"
      ref="MultiServerCar"
    ...
    <!-- @change="handleCarouselChange" -->
      <el-carousel-item>
        ...
        <div>
          <YieldChartLine
            ref="DetailYield7WeekM"
            style="height: 100%"
            cid="DetailYield7WeekM"
          />
        </div>
        <!-- Pareto(CUM) -->
        <div>
      ...
      <el-carousel-item>
        ...
        <div>
          <YieldChartLine
            ref="DetailYield7DaysTV"
            style="height: 100%"
            cid="DetailYield7DaysTV"
          />
        </div>
      ...
      <el-carousel-item>
        <div>
          <DefectRateChartLine
            :ref="`DefectRateChartLineM`"
            style="height: 100%"
            :cid="`DefectRateChartLineM`"
          />
        </div>
        <div>
          <DefectByColor
...
import { multiServer, multiServerForm } from '@/config'
...
export default {
  ...
  created() {
   /*  this.devServer.forEach((key) => {
      this.title[key] = key.slice(key.lastIndexOf('_') + 1)
    }) */
  },
  mounted() {
   /*  this.handleServerSwitch()
    ...
    }, 3 * this.devServer.length * this.interval) */
  },
  beforeDestroy() {
  ...
  methods: {
    ...
    handleGetToken(data, api) {
      ...
      getToken(data, api).then((res) => {
        if (res.Success) {
          ...
          this.$store
            .dispatch('user/loginNoKey', { userInfo: this.form, api })
            .then(() => {
    ...
    handleServerSwitch() {
      let index = 0,
        that = this
      that.objData[that.devServer[index] + 'GetUPHYieldByDay'] = null
      that.objData[that.devServer[index] + 'GetUPHYieldByWeek'] = null
      that.objData[that.devServer[index] + 'GetDefectYield'] = null
      ...
      this.time = setInterval(function () {
        ...
        index++
        if (index >= that.devServer.length) {
