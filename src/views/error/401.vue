      ...
      <el-row :gutter="20">
        ...
        <el-col :xs="24" :sm="24" :md="12" :lg="12" :xl="12">
          <div class="bullshit">
            ...
            <a class="bullshit-return-home" @click="gotoHome">
              {{ jumpTime }}s&nbsp;{{ btn }}
            </a>
            <span class="reset_login" @click="handleLogin">{{ $t('loginAgain') }}</span>
          </div>
        </el-col>
      </el-row>
    </div>
...
export default {
  name: 'Page401',
  data() {
    let p = this.$store.getters['user/asyncPath']
    return {
      jumpTime: 3,
      oops: this.$t('sorry'),
      headline: this.$t('YouDoNotHavePermissionToOperate'),
      info: this.$t('PleaseContactTheAdministrator'),
      btn: this.$t('returnHome'),
      timer: 0,
      path: p ? p : '/login',
    }
  },
  ...
  methods: {
    gotoHome() {
      //为公共页面跳转
      clearInterval(this.timer)
      if (this.$store.getters['user/isPublicPage']) {
