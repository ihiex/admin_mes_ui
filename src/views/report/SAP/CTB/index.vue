<template>
  ...
  <ZReportQuery ref="ZReportQueryRef" v-loading="loading" :total="total" :page-size="form.PageSize" :show-pagination="true"
    :current-page-index="form.CurrentPageIndex" @search="handleSearch" @reset="handleReset">
    <el-button slot="button" v-hasPermi="['SAP/CTB/ExportData']" :icon="downLoading ? 'el-icon-loading' : 'el-icon-download'" :disabled="downLoading"
    ...
    <div slot="table" style="height: 100%;">
      <el-table :data="tableData" style="width: 100%;" :height="height" border>
        <el-table-column v-for="item in tableHead" :key="item" :prop="item" :label="item" sortable min-width="150"
