<template>
  <div v-loading="loading" class="dashboard" :style="{ backgroundImage: `url(/assets/images/${bgImage}.jpeg)` }">
    <div class="header">
      ...
      <span style="position: absolute; right: 50px; font-size: 16px; top: 15px" :style="{ color: color }">
        {{ $store.state.reportParams.paramsV2.S_EndDateTime }}
      </span>
      <el-popover v-model="visible" style="position: absolute; left: 15px; top:0px;z-index: 101;" width="900"
        placement="bottom" trigger="click">
        <div>
          <query-condition :key="qKey" v-model="$store.state.reportParams.paramsV2"
          ...
          <div style="text-align: right; padding-top: 10px;">
            ...
            <el-switch v-model="$store.state.reportParams.isAutoV2" :active-text="$t('Auto')"
        ...
        <el-button slot="reference" icon="el-icon-search" circle type="text" style="position: relative; top: 5px"></el-button>
      </el-popover>
    </div>
    <el-container style="margin-bottom: 0; padding-bottom: 0; ">
      ...
      <el-main id="M_header" v-loading="oneEven" indicator-position="none"
            ...
            <el-dropdown-menu slot="dropdown">
              <div style="width: 200px; padding-left: 15px;">
                <div>
                  ...
                  <el-input v-model="$store.state.reportParams.paramsV2.S_TopQTY" type="number" style="width: 150px"
        ...
        <div :id="ids + 'M_footer'" style="height: 96%;width: 96%;" class="shadow_style">
        </div>
      </el-main>
    </el-container>
...
import { getData } from '@/views/report/production/dashboard/data'
...
export default {
  ...
  data() {
    ...
    let color =localStorage.getItem('DashboardV2FontColor');
    let barColor = localStorage.getItem('DashboardV2BarColor');
    let bgImage = localStorage.getItem('DashboardV2BgImage');
    return {
  ...
  watch: {
    ...
    '$store.state.reportParams.isAutoV2': function (val, oval) {
      if (val) {
        this.handleReset()
      }
    }
  ...
  methods: {
    ...
    handleReset() {
      let shift = getShift(this.$store.getters['user/shiftTable'])
      let s = getUserShift(this.$store.getters['user/shiftTable']);
      let params = this.$store.getters['reportParams/paramsV2']
      this.$set(params, 'S_StartDateTime', shift.sTime)
      this.$set(params, 'S_EndDateTime', new Date().format('yyyy-MM-dd hh:mm:ss'))
    ...
    handleRefreshData() {
      ...
      that.time = setInterval(function () {
        if (that.$store.state.reportParams.isAutoV2) {
          ...
          that.$store.dispatch('reportParams/setParamsV2', params)
    ...
    handleQuery(isShow) {
      ...
      this.dyData = {}
      // debugger
      if (this.$store.state.reportParams.isAutoV2) {
        let shift = getShift(this.$store.getters['user/shiftTable'])
        let s = getUserShift(this.$store.getters['user/shiftTable']);
    ...
    getData(isShow) {
      let params = JSON.parse(JSON.stringify(this.$store.state.reportParams.paramsV2))
      ...
      this.one = true
      // 清空视图
      this.chartCxt[this.dyId] && this.chartCxt[this.dyId].dispose()
    ...
    getDefectYield(param, key) {
      ...
      GetDefectYield(params, '').then((res) => {
        ...
        if (res.Success && data) {
          ...
          this.handleDefectChart(this.dyData[lineName], this.dyId, yieldCumActual, this.processYield[lineName].target, this.$store.state.reportParams.paramsV2.S_TopQTY, lineName, Object.keys(this.carouselData?.[lineName] ?? {})?.[0]);
          // }
        }
        this.oneEven = false
    ...
    handleCumTargetInput(val) {
      ...
      this.handleDefectChart(this.dyData[lineName], this.dyId, yieldCumActual, val, this.$store.state.reportParams.paramsV2.S_TopQTY, lineName, Object.keys(this.carouselData?.[lineName] ?? {})?.[0]);
