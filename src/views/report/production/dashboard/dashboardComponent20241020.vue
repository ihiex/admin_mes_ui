    ...
    <el-container style="margin-bottom: 0; padding-bottom: 0;">
      <el-header height="50%" style=" padding: 0;margin: 0;" :style="{ height: 'calc(' + headerHeight + ' / 2)' }">
        <el-container style="height: inherit; padding: 0; margin: 0;">
          ...
          <el-aside width="38%" style="overflow: hidden; display: flex;">
      ...
      <el-footer style="padding: 0;margin: 0; position: relative;"
        ...
        <el-container style="height: 98%; padding: 0; margin: 0; position: relative;">
          <el-aside width="33%" style="overflow: hidden; display: flex;">
          ...
          <el-main style="overflow: hidden; display: flex; margin: 0; padding: 0;">
  ...
  methods: {
    ...
    handleRightFooterChart(id, title, data, rotate = 80) {
      ...
      data.data.forEach(val => {
        list.push({ value: val, itemStyle: { color: !data.target ? this.color : parseFloat(val) < data.target ? '#ad2424' : '#6790d8' } })
      })
        ...
        series: [
          {
            ...
            label: {
              formatter: [
                ...
                '{value|Actual:' + (data.Yield ? data.Yield : '--') + '% ' + handleCompare(data.yTarget, data.Yield) + '}\n',
                '{hr|}\n',
                // '{head|Output}',
          ...
          },
          {
            ...
            type: 'pie',
            radius: ['75%', '85%'],
            // radius: ['30%', '45%'], //这两个数字相差越小 环的宽度越窄 相差越大条越宽
            ...
            data: [
              {
                ...
                name: '',
                itemStyle: {
                  color: '#5470c6', //进度完成部分填充颜色
                  borderRadius: 100
                },
              },
              {
                value: 100 - (data.Output / data.target * 100),
                name: '',
                itemStyle: {
                  color: '#e9e9e9'
                }
    ...
    getDefectYield(isShow) {
      ...
      GetDefectYield(params, '', isShow).then((res) => {
        let data = res.ResData
        if (res.Success && data) {
          let list = {
