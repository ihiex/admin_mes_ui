<template>
  <div class="dashboard" :style="{ backgroundImage: `url(/assets/images/${bgImage}.jpeg)` }">
    <div class="header">
      ...
      <span style="position: absolute; right: 50px; top: 17px; font-size: 16px;" :style="{ color: color }">
        {{ $store.state.reportParams.params.S_EndDateTime }}
      </span>
      <el-popover v-model="visible" style="position: absolute; left: 15px; top:0px;z-index: 101;" width="900"
        placement="bottom" trigger="click">
        <div>
          <query-condition :key="qKey" v-model="$store.state.reportParams.params"
          ...
          <div style="text-align: right; padding-top: 10px;">
            ...
            <el-switch v-model="$store.state.reportParams.isAuto" :active-text="$t('Auto')"
        ...
        <el-button slot="reference" icon="el-icon-search" circle type="text"
          style="position: relative; top: 5px"></el-button>
      </el-popover>
    </div>
    <el-container style="margin-bottom: 0; padding-bottom: 0;">
      <el-header height="50%" style=" padding: 0;margin: 0;" :style="{ height: 'calc(' + headerHeight + ' / 2)' }">
        <el-container style="height: inherit; padding: 0; margin: 0; position: relative;">
          <div class="target_by_hour_input">
            <el-dropdown>
              ...
              <el-dropdown-menu slot="dropdown">
                ...
                <el-input v-model="targetByHour" type="number" style="width: 80px"
                  @input="handleTargetByHourInput"></el-input>
              </el-dropdown-menu>
            </el-dropdown>
          </div>
          ...
          <div class="top_input">
            <el-dropdown>
              ...
              <el-dropdown-menu slot="dropdown">
                ...
                <el-input v-model="$store.state.reportParams.params.S_TopQTY" type="number" style="width: 80px"
...
export default {
  ...
  data() {
    ...
    let color = localStorage.getItem('DashboardFontColor');
    let barColor = localStorage.getItem('DashboardBarColor')
    let target = localStorage.getItem('YieldTarget');
    let targetByHour = localStorage.getItem('TargetByHour');
    let bgImage = localStorage.getItem('DashboardBgImage');
    ...
    return {
      ...
      barColor: barColor ? barColor : '#71aeed',
      fontSize: 14,
      titleFontSize: 16,
      time: null,
      target: target ? target : 95,
      targetByHour: targetByHour ? targetByHour : 270,
      carouselData: {},
      processYield: {},
      outputData: {},
      targetOutput: {},
  ...
  watch: {
    ...
    '$store.state.reportParams.isAuto': function (val, oval) {
      if (val) {
        this.handleReset()
      }
    }
  ...
  methods: {
    ...
    // 手输uph 目标值
    handleTargetByHourInput(val) {
      if (isNaN(val)) return;
      localStorage.setItem('TargetByHour', val);

      Object.values(this.targetOutput).forEach(values => {
        Object.values(values).forEach(item => {
          item?.target?.forEach((res, index) => {
            if (!item.srcFlag) {
              item.target[index] = parseInt(val)
            } else {
              return
            }
          })
    ...
    handleTargetInput(val) {
      ...
      Object.keys(this.carouselData).forEach(key => {
        Object.keys(this.carouselData[key]).forEach(k => {
          if (!this.carouselData[key][k].srcFlag) {
            this.carouselData[key][k]['yTarget'] = parseFloat(val)
          } else {
            return
          }
        })
    ...
    setInterval() {
      ...
      if (that.$store.state.reportParams.isAuto) {
        that.time = setInterval(function () {
          let shift = getShift(that.$store.getters['user/shiftTable'])
          let s = getUserShift(that.$store.getters['user/shiftTable']);
          let params = that.$store.state.reportParams.params

          that.$set(params, 'S_EndDateTime', new Date().format('yyyy-MM-dd hh:mm:ss'))
          that.$set(params, 'S_Shift', s)
          that.$set(params, 'S_StartDateTime', shift.sTime)
          that.$store.dispatch('reportParams/setParams', params)
    ...
    handleReset() {
      ...
      this.$set(params, 'S_DataType', '0')
      this.$set(params, 'S_PartFamilyTypeID', null) //料号类别
      this.$set(params, 'S_PartFamilyID', null) //料号组
      this.$set(params, 'S_PartID', null) //料号
      this.$set(params, 'S_ProductionOrderID', null) //工单
      this.$set(params, 'S_StationTypeID', null) //工站类型
      this.$set(params, 'S_StationID', null) //工站
      this.$set(params, 'S_LineID', null) //产线
      this.$set(params, 'S_Shift', s) //班次
      this.$set(params, 'S_DataLevel', 4) //
    ...
    handleQuery(isShow) {
      ...
      this.outputData = {}
      this.targetOutput = {}
      if (this.$store.state.reportParams.isAuto) {
        let shift = getShift(this.$store.getters['user/shiftTable']);
        let s = getUserShift(this.$store.getters['user/shiftTable']);
        let params = this.$store.state.reportParams.params;
        ...
        series: [
          {
            ...
            label: {
              formatter: [
                ...
                '{value|CompleteRate:' + (data.target == 0 ? '--' : (data.Output / data.target * 100).toFixed(0)) + '%}\n',
                '{value|Output}',
              ].join('\n'),
              borderRadius: 4,
              position: 'center',
    ...
    getData(isShow) {
      let params = JSON.parse(JSON.stringify(this.$store.state.reportParams.params))
      ...
      Object.keys(this.chartCxt).forEach(key => {
        this.chartCxt?.[key] && this.chartCxt?.[key]?.clear()
      })
      ...
      Object.keys(obj).forEach(key => {
        ...
        obj[key].forEach(item => {
          ...
          if (!o.axis.includes(item.StationType)) {
            ...
            o['target'] = item.YieldTarget || parseFloat(this.target);
          }

          // carousel
          if (!c[item.StationType]) {
            c[item.StationType] = {}
            c[item.StationType]['srcFlag'] = item.YieldTarget ? true : false;
            c[item.StationType]['yTarget'] = item.YieldTarget || parseFloat(this.target);
        ...
        Object.keys(toObj[key]).forEach(k => {
          ...
          Object.keys(toObj[key][k].Output).forEach(kk => {
            if (new Date(kk) != 'Invalid Date' || kk.includes('WK')) {
              ...
              let t = toObj[key][k].UPHTarget[kk] || toObj[key][k].UPHTargetByHour[kk] || parseInt(this.targetByHour)
              target.push(t)
              data.push(toObj[key][k].Output[kk])
            }
          })
          toObj[key][k] = { yAxis, target, data, srcFlag }
        })
      })
