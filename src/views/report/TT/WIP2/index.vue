<template>
  <div class="page_container">
    <div class="query_bar">
      ...
      <div class="query_btn">
        <el-button v-hasPermi="['TT/WIP2/ExportData']" :icon="loading ? 'el-icon-loading' : ''" :disabled="loading" size="mini" @click="handleExport">
          {{ $t('export') }}
        </el-button>
    ...
    <div class="chart_box">
      <div :class="{ table_content: !show, table_content_show: show }">
        <TTReportWIP2 ref="TTReportWIP2" style="height: 100%" />
      </div>
      <div class="link_detail">
        <span v-if="total >= 0" style="margin-right: 20px; font-size: 16px">
          Total:{{ total }}
        </span>
        <a @click="gotoDetail">{{ $t('WIP2List') }}</a>
      </div>
    </div>
  </div>
</template>
...
import TTReportWIP2 from '@/components/chart/TTReportWIP2.vue'
...
export default {
  ...
  watch: {
    collapse: function (val, oval) {
      this.isActivePage && this.$refs.TTReportWIP2.handleWindowResize()
    },
  ...
  activated() {
    ...
    this.$refs.TTReportWIP2.handleWindowResize()
  ...
  methods: {
    ...
    handleSearch(isShowLoading) {
      ...
      this.total = 0
      this.isActivePage && this.$refs.TTReportWIP2.show([])
      GetReportGeneral(params, isShowLoading).then((res) => {
        if (res.Success) {
          ...
          this.isActivePage && this.$refs.TTReportWIP2.show(res.ResData[0])
        }
      })
    },
  },
