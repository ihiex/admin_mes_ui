                      ...
                      <div class="sn_container_left">
                        <div>
                          <el-form ref="ruleForm" :model="query" :rules="rules" label-width="100px" class="demo-ruleForm" @submit.native.prevent>
                            <!-- 新治具条码 -->
                            <el-form-item :label="$t('NewToolingSN')" prop="S_NewToolingSN">
                              <el-input ref="BoxSNInput" v-model.trim="query.S_NewToolingSN" autocomplete="off" clearable
                                :disabled="isLockBoxSN" style="width: 99%"
                                @keypress.native.enter="handleBoxSNEnter(query.S_NewToolingSN, 'ruleForm')"></el-input>
                            </el-form-item>
                            <!-- 旧治具条码 -->
                            <el-form-item :label="$t('OldToolingSN')" prop="S_OldToolingSN" style="margin-bottom: 0;">
                              <el-input ref="snInput" v-model.trim="query.S_OldToolingSN" autocomplete="off"
                                :disabled="snDisabled" clearable style="width: 99%; margin-bottom: 10px;"
                                @keypress.native.enter="handleSNEnter(query.S_OldToolingSN, 'ruleForm')">
                              </el-input>
                            </el-form-item>
                            <el-form-item :label="' '" style="text-align: right;">
                              <el-button plain style="margin: 0 10px;" @click="handleRelease('ruleForm')">{{
                                $t('Release')
                              }}</el-button>
                            </el-form-item>
                          </el-form>
...
import {
  ToolGetPageInitializeAsync,
  ToolSetConfirmPOAsync,
  ToolNewToolingSnVerifyAsync,
  ToolOldToolingSnVerifyAsync,
  ToolOldToolingSnReleaseAsync
...
export default {
  ...
  data() {
    let validateNewTooling = (rule, value, callback) => {
      if (!value || value == '') {
        callback(new Error(this.$t('TheBarCodeOfTheNewFixtureCannotBeEmpty')));
      } else {
        callback();
      }
    };
    let validateOldTooling = (rule, value, callback) => {
      if ((!value || value == '') && !this.snDisabled) {
        callback(new Error(this.$t('TheBarCodeOfTheOldFixtureCannotBeEmpty')));
      } else {
        callback();
      }
    };
  ...
  methods: {
    ...
    handleRelease(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$confirm(this.$t('UnlockingTheCurrentFixture'), this.$t('hint'), {
            confirmButtonText: this.$t('confirm'),
            cancelButtonText: this.$t('cancel'),
            type: 'warning'
          }).then(() => {
            ToolOldToolingSnReleaseAsync(this.formatData(this.query)).then(res => {
              console.log(res)
              try {
                if (res.ResData && res.Success) {
                  ...
                  this.$set(this.query, "S_OldToolingSN", "");
    ...
    GetPageInitialize() {
      // debugger
      ToolGetPageInitializeAsync({ S_URL: this.$route.path }).then((res) => {
        try {
    ...
    handleQueryReset() {
      ...
      this.$set(this.query, "S_OldToolingSN", "");
      this.$set(this.query, "S_NewToolingSN", "");
    ...
    handleBoxSNEnter(val, formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          ToolNewToolingSnVerifyAsync(this.formatData(this.query), false, true).then(
            (res) => {
              try {
                if (res.Success && res.ResData) {
                  this.hintInfos.unshift({
    ...
    handleSNEnter(val, formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ToolOldToolingSnVerifyAsync(this.formatData(this.query), false, true).then(
            (res) => {
              try {
                if (res.Success && res.ResData) {
                  this.hintInfos.unshift({
    ...
    handleConfirm(query) {
      ...
      this.target = 0;
      // debugger
      ToolSetConfirmPOAsync(params, true, true).then(
        (res) => {
          try {
