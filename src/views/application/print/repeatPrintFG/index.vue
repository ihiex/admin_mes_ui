                ...
                <transition name="slide-fade">
                  <div>
                    <MesQuery ref="mesQuery" v-model="query" :disabled-confirm="disabledConfirm" :is-disable="isDisable"
                      :is-show-work-order="true" :is-show-line="false" :is-show-status="false"
                        ...
                        <div style="width: 92%;">
                          <el-form ref="ruleForm" :model="query" :rules="rules" label-width="100px" class="demo-ruleForm">
                            ...
                            <el-form-item :label="$t('SN')" prop="S_SN">
                              <el-input ref="snInput" v-model.trim="query.S_SN" autocomplete="off" clearable @keyup.native.enter="handleSNEnter(query.S_SN)">
                              </el-input>
                            </el-form-item>
                          </el-form>
                          <el-row>
...
export default {
  ...
  methods: {
    ...
    print() {
      this.$set(this.query, 'Type', 'FG')
      ReFGSetScanSN(this.query).then(res => {
        this.$set(this.query, 'S_SN', '');
        let data = res.ResData
        if (res.Success && data) {
          this.SNList = data.ListSN
          //格式 "PRINT|工站|SN|模板|料号"
          let printStr = `PRINT|${this.$store.getters['stationSet/currentStation'].Description}|${data.ListSN.join(',')}|${data.LabelPath}|${this.query.S_PartID}`
    ...
    handleConfirm(query) {
      ...
      this.$set(params, 'Type', 'FG')
      this.BOMtableData = []
      this.craftTableData = []
      ReFGSetConfirmPO(params, true, true).then(
        (res) => {
    ...
    GetPageInitialize() {
      ReFGGetPageInitializeAsync({ S_URL: this.$route.path }).then((res) => {
        try {
