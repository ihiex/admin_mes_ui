                      ...
                      <div class="sn_container_left">
                        ...
                        <div style="width: 92%;">
                          <el-form ref="ruleForm" :model="query" :rules="rules" label-width="110px" class="demo-ruleForm" @submit.native.prevent>
                            ...
                            <el-form-item :label="$t('ScanOtherSN', { 0: $t('Fixture') })" prop="S_SN">
...
import {
  FixturePrintSetScanSN,
  FixturePrintGetPageInitialize,
  FixturePrintSetConfirmPO,
...
export default {
  ...
  methods: {
    ...
    handleCreateAndPrint(formName, val) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          FixturePrintSetScanSN(this.query).then(res => {
    ...
    handleConfirm(query) {
      ...
      this.BOMtableData = []
      this.craftTableData = []
      FixturePrintSetConfirmPO(params, true, true).then(
        (res) => {
          if (res.Success && res.ResData) {
    ...
    GetPageInitialize() {
      FixturePrintGetPageInitialize({ S_URL: this.$route.path }).then((res) => {
        try {
