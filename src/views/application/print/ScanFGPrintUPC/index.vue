                      ...
                      <div class="sn_container_left">
                        ...
                        <div style="width: 92%;">
                          <el-form ref="ruleForm" :model="query" :rules="rules" label-width="100px" class="demo-ruleForm" @submit.native.prevent>
                            ...
                            <el-form-item :label="$t('ScanOtherSN', { 0: 'FG' })" prop="S_SN">
...
import {
  ScanFGPrintUPCSetScanSN,
  ScanFGPrintUPCGetPageInitialize,
  ScanFGPrintUPCSetConfirmPO,
...
S12
S12_FG
Part_S12_FG_SS22_Orange2(橙2)
PO_S12_KIT_FE_Orange2(FE橙2)
...
export default {
  ...
  methods: {
    ...
    handleCreateAndPrint(formName, val) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          ScanFGPrintUPCSetScanSN(this.query).then(res => {
    ...
    handleConfirm(query) {
      ...
      this.BOMtableData = []
      this.craftTableData = []
      ScanFGPrintUPCSetConfirmPO(params, true, true).then(
        (res) => {
          if (res.Success && res.ResData) {
    ...
    GetPageInitialize() {
      ScanFGPrintUPCGetPageInitialize({ S_URL: this.$route.path }).then((res) => {
        try {
