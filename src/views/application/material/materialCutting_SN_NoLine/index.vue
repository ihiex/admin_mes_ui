...
export default {
  ...
  data() {
    return {
      ...
      params: {
        BatchNo: '',
        Type: 'BatchToSN_NotLineNumber', //SmallBatch（大批次切小批次）BatchToSN(批次切SN) BatchToSNDOE(批次切SN DOE) BatchToSN_NotLineNumber(批次切SN 无线号)
        RollNO: '',
        SpecQTY: null,
        UsageQTY: null,
        BatchQTY: null,
  ...
  methods: {
    ...
    handleSplit() {
      ...
      delete this.params.S_LineNumber
      this.tableData = [];
      this.snLoading = true
    ...
    handleQueryReset() {
      ...
      Object.keys(this.params).forEach(key => {
        if (key === 'Type') {
          this.$set(this.params, key, 'BatchToSN_NotLineNumber')
    ...
    handleConfirm(query) {
      ...
      this.$set(params, 'Type', 'BatchToSN_NotLineNumber');
      this.BOMtableData = [];
      this.craftTableData = [];
      this.completed = 0;
      this.target = 0;
