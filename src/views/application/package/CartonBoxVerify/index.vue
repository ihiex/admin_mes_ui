                                ...
                                <div style="text-align: right;">
                                  ...
                                  <!--   <span :style="{ color: serialMsg.flag ? '#85ce62' : '#be4036' }">{{ serialMsg.message
                                    ...
                                    <serial-button ref="SerialButton" style="margin-left: 5px;"
                                      ...
                                      @SerialRead="handleSerialRead" @error="handleSocketError" @success="handleSocketMsg" @IsConnectPrint="handleConnectPrint">
                                    </serial-button>
                                  </el-form-item>
...
import {
  PackageGetPageInitializeAsync,
  PackageSetConfirmPOAsync,
  PackageMainSnVerifyAsync,
  PackageDynamicSnVerifyAsync,
  PackageRemoveSingleAsync,
  PackageReprintBoxSnAsync,
  PackageLastBoxSubmitAsync
} from "@/api/application.js";
import RemoveDialog from './remove.vue'
//中箱校验
/* 
箱号
V3,SSCC008859096851338885,GTIN00194252780657,SCC30194252780658,MPNMM213ZM/A,QTY10,S,S,S,S,S,S,S,S,S,S
产品
COO:CN,sG56GN03H192M
COO:CN,sG56GN03N192M

COO:CN,sG56GN05J192M
COO:CN,sG56GN05Q192M

COO:CN,sG56GN060192M
COO:CN,sG56GN06B192M

COO:CN,sG56GN06Z192M
COO:CN,sG56GN074192M

COO:CN,sG56GN07X192M
COO:CN,sG56GN08P192M
无箱号：
COO:CN,sG56GN03Z192M
COO:CN,sG56GN049192M

COO:CN,sG56GN05R192M
COO:CN,sG56GN05T192M

COO:CN,sG56GN05Z192M
COO:CN,sG56GN06P192M

COO:CN,sG56GN077192M
COO:CN,sG56GN081192M

COO:CN,sG56GN08A192M
COO:CN,sG56GN08K192M
...
export default {
  ...
  methods: {
    handleDialogConfirm(data) {
      ...
      PackageRemoveSingleAsync(params).then(res => {
        try {
    ...
    handlePrint() {
      this.$prompt(this.$t('PleaseScanOrEnterTheReprintedBoxCode'), this.$t('hint'), {
        ...
        PackageReprintBoxSnAsync(params).then(res => {
          try {
            let data = res.ResData
            if (res.Success && data) {
              this.hintInfos.unshift({
    ...
    handleLastBox() {
      this.$confirm(this.$t('WhetherToPerformTailBoxOperations'), this.$t('hint'), {
        ...
        PackageLastBoxSubmitAsync(params).then(res => {
          try {
            let data = res.ResData
            if (res.Success && data) {
              this.hintInfos.unshift({
    ...
    //初始化
    GetPageInitialize() {
      PackageGetPageInitializeAsync({ S_URL: this.$route.path }).then((res) => {
        try {
    ...
    handleMainSn(val, formName) {
      if (!val) return
      PackageMainSnVerifyAsync(this.formatData(this.query), false, true).then(
        (res) => {
          try {
            let data = res.ResData
            if (res.Success && data) {
    ...
    handleDynamicSn(val, formName, key) {
      ...
      let params = this.formatData(JSON.parse(JSON.stringify(this.query)));
      PackageDynamicSnVerifyAsync(params, false, true).then(
        (res) => {
          try {
            let data = res.ResData
            if (res.Success && data) {
    ...
    handleConfirm(query) {
      ...
      this.completed = 0;
      this.target = 0;
      PackageSetConfirmPOAsync(params, true, true).then(
        (res) => {
          try {
            let data = res.ResData;
            if (res.Success && data) {
              ...
              if (poAttributes.IsGenerateBoxSN === '1') {
                ...
                //初始化打印服务
                this.$nextTick(function () {
                  this.$refs.PrintButton.init(this.socketAddress/* '172.16.32.68:55210' */)
                })
              } else {
                this.isGenerateBoxSN = false;
                this.isLockBoxSN = false;
