  ...
  <div v-if="IsLegalPage" class="app_container">
        ...
        <el-form ref="form" :model="form" label-width="80px">
          <el-form-item :label="$t('date')" style="display: inline-block;">
            <el-date-picker v-model="form.date" type="daterange" align="right" style="width: 220px;" unlink-panels
              ...
              :end-placeholder="$t('endDate')" :picker-options="pickerOptions" @change="handleDateChange">
            </el-date-picker>
          </el-form-item>
          ...
          <el-button v-hasPermi="['billShipment/Print']" v-loading="printLoading"
            element-loading-spinner="el-icon-loading" size="small" plain @click="handlePrint">{{ $t('Print')
            }}</el-button>
          <el-button v-hasPermi="['billShipment/Add']" size="small" plain @click="handleAdd">{{ $t('Added') }}</el-button>
          <el-button v-hasPermi="['billShipment/Audit']" size="small" plain @click="handleAudit">{{ $t('Audit')
          }}</el-button>
          <el-button v-hasPermi="['billShipment/Shipped']" size="small" plain @click="handleShipped">{{ $t('Shipped')
          }}</el-button>
        </el-form>
      </el-row>
    </div>
    <div id="BillShipmentMain" v-loading="loading" class="main">
      <el-table ref="multipleTableBody" :data="tableData" tooltip-effect="dark" style="width: 100%"
        ...
        <el-table-column label="FBill NO" prop="FBillNO" min-width="270" sortable show-overflow-tooltip fixed="left">
        </el-table-column>
    ...
    <div class="footer">
      ...
      <el-table ref="multipleTable" v-loading="childLoading" :data="childTableData" tooltip-effect="dark"
        ...
        <el-table-column :label="$t('operation')" width="100" fixed="left" align="center">
          <template slot-scope="scope">
            <el-button v-if="isModifyChild" type="info" circle size="mini" icon="el-icon-edit"
              @click="handleModifyChild(scope.row, currentRow.FInterID)"></el-button>
            <el-button v-if="isDeleteChild" type="warning" circle size="mini" icon="el-icon-delete"
              @click="handleDeleteChild(scope.row.FDetailID, currentRow.FInterID)"></el-button>
          </template>
        </el-table-column>
...
<script>
import {
  ShipmentGetPageInitialize,
  ShipmentGetWHType, //状态数据类别
  ShipmentGetShipmentNew, // 获取主数据
  ShipmentGetShipmentEntryNew, // 获取子数据
  ShipmentGetShipmentReport, // 栈板纸
  ShipmentSetShipmentNew, // 编辑主表
  ShipmentUpdateFStatus, // 批量更新状态
  ShipmentDelShipmentNew, // 删除主表
  ShipmentSetShipmentEntryNew, //编辑子表
  ShipmentDelShipmentEntryNew // 删除子表
  ...
  methods: {
    ...
    handleDelete(id) {
      this.$confirm(this.$t('WhetherToPerformTheDeletionOperation'), this.$t('hint'), {
        ...
        ShipmentDelShipmentNew({ FInterID: id }).then(res => {
          if (res.Success) {
            //从前端移除
    ...
    handleRowClick(row, column, event) {
      ...
      ShipmentGetShipmentEntryNew({ S_FInterID: row.FInterID }, false).then(res => {
    ...
    handleConfirm(params) {
      ShipmentSetShipmentNew(params).then(res => {
        if (res.Success) {
          this.handleSearch()
        }
      })
    ...
    handleChildConfirm(params) {
      console.log(params)
      ShipmentSetShipmentEntryNew(params).then(res => {
        if (res.Success) {
          this.handleSearch()
        }
      })
    ...
    handleDeleteChild(DetailID, FInterID) {
      this.$confirm(this.$t('WhetherToDeleteChildItems'), this.$t('hint'), {
        ...
        ShipmentDelShipmentEntryNew({ S_FDetailID: DetailID, S_FInterID: FInterID }).then(res => {
          if (res.Success) {
    ...
    handleSearch() {
      ...
      this.loading = true;
      this.sum = 0
      ShipmentGetShipmentNew(this.form).then(res => {
        this.loading = false
        if (res.Success && res.ResData) {
    ...
    handlePrint() {
      ...
      ShipmentGetShipmentReport(this.form).then(res => {
        try {
          if (res.Success && res.ResData) {
    ...
    handleAudit() {
      ...
      this.$confirm(this.$t('AuditConfirm'), this.$t('hint'), {
        ...
        this.handleBatchUpdate({ FStatus: 1, FInterID_List: str })
      }).catch(() => {
      });
    ...
    handleShipped() {
      ...
      this.$confirm(this.$t('ShippedConfirm'), this.$t('hint'), {
        ...
        if (list.length <= 0) return
        let str = list.map(res => res.FInterID).join(',')
        this.handleBatchUpdate({ FStatus: 3, FInterID_List: str })
      }).catch(() => {
      });
    },
    handleBatchUpdate(params) {
      ShipmentUpdateFStatus(params).then(res => {
        if (res.Success) {
          //刷新页面
          this.handleSearch()
        }
    ...
    handleInitPage() {
      ShipmentGetPageInitialize({ S_URL: this.$route.path }).then(res => {
        try {
