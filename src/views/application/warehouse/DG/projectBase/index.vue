<template>
  <div v-loading="loading" class="page_container">
    <z-query ref="ZQuery" v-model="form.Keywords" style="height: inherit;" :show-add="showAdd" :total="total"
      ...
      <div slot="table" style="height: 100%;">
        <el-table :data="tableData" style="width: 100%;" height="100%">
          ...
          <el-table-column prop="FCountByPallet" :label="$t('FullPalletQty')" sortable min-width="140"></el-table-column>
          ...
          <el-table-column prop="" :label="$t('operation')" fixed="right" width="130">
            <template slot-scope="props">
              <el-button v-hasPermi="['projectBase/EditDG']" size="mini" type="primary" icon="el-icon-edit" circle
                @click="handleEdit(props.row)"></el-button>
              <el-button v-hasPermi="['projectBase/DeleteDG']" size="mini" type="danger" icon="el-icon-delete" circle
                @click="handleDelete(props.row)"></el-button>
                <el-button v-hasPermi="['projectBase/EditDG']" size="mini" icon="el-icon-copy-document" circle
...
import { DGProjectBaseDeleteAsync, DGProjectBaseFindWithPagerAsync } from '@/api/application.js'
...
export default {
  ...
  mounted() {
    ...
    let res = per.some(r => r === 'projectBase/AddDG')
    if (res) {
      this.showAdd = true
    } else {
      this.showAdd = false
  ...
  methods: {
    ...
    handleSearch(item) {
      ...
      if (item && JSON.stringify(item) != '{}') {
        Object.keys(item).forEach(key => {
          if (key === 'LikeQuery') {
            this.$set(this.form, 'Keywords', item[key])
          } else {
    ...
    getData() {
      this.loading = true
      DGProjectBaseFindWithPagerAsync(this.form).then(res => {
        this.loading = false
        try {
    ...
    handleDelete(val) {
      this.$confirm(this.$t('delete') + this.$t('affirm'), this.$t('hint'), {
        ...
        DGProjectBaseDeleteAsync(val).then(res => {
          if (res.Success) {
            this.handleSearch()
          }
        })
