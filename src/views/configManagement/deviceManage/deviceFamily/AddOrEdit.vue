  ...
  <el-dialog :title="title" :visible.sync="dialogVisible" width="400" :close-on-click-modal="false"
    ...
    <div v-loading="loading">
      <el-form ref="form" :model="form" :rules="rules" label-width="120px">
        ...
        <el-form-item :label="$t('DeviceFamilyType')" prop="MachineFamilyTypeID">
          <el-select v-model="form.MachineFamilyTypeID" :placeholder="$t('pleaseSelect')" style="width: 100%;" filterable
            clearable>
...
<script>
import {
  MachineFamilyInsertAsync,
  MachineFamilyUpdateAsync,
    ...
    confirm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.loading = true
          if (this.flag === 'Edit') {
            MachineFamilyUpdateAsync(this.form).then(res => {
              if (res.Success) {
            ...
            MachineFamilyInsertAsync(this.form).then(res => {
              if (res.Success) {
                this.$emit('confirm', this.flag)
              }
              this.loading = false
