<template>
  <div v-loading="loading" class="page_container">
    <z-query ref="ZQuery" v-model.trim="form.Keywords" style="height: inherit;" :show-add="showAdd" :total="total"
      ...
      <el-button slot="button" v-hasPermi="['deviceFamilyType/BeachDelete']" type="danger" icon="el-icon-delete"
          ...
          <el-row :gutter="10">
            <el-col :xs="24" :sm="12" :md="8" :lg="8" :xl="8">
              <el-form-item :label="$t('name')">
                <el-input v-model.trim="form.MachineFamilyTypeName" :placeholder="$t('pleaseInput')" clearable></el-input>
              </el-form-item>
            </el-col>
        ...
        <el-table v-if="update" :data="tableData" style="width: 100%;" :default-expand-all="expandAll" height="100%"
          ...
          <el-table-column prop="" :label="$t('operation')" width="100" fixed="right">
            <template slot-scope="props">
              <el-button v-hasPermi="['deviceFamilyType/Edit']" size="mini" type="primary" icon="el-icon-edit" circle
                @click="handleEdit(props.row)"></el-button>
              <el-button v-hasPermi="['deviceFamilyType/Delete']" size="mini" type="danger" icon="el-icon-delete" circle
                @click="handleDelete(props.row)"></el-button>
              <!-- <el-button v-hasPermi="['deviceFamilyType/Clone']" size="mini" icon="el-icon-copy-document" circle
...
<script>
import {
  MachineFamilyTypeDeleteAsync,
  MachineFamilyTypeFindWithPagerSearchAsync,
} from '@/api/config.js'
...
export default {
  ...
  mounted() {
    ...
    let res = per.some(r => r === 'deviceFamilyType/Add')
    if (res) {
      this.showAdd = true
    } else {
      this.showAdd = false
  ...
  methods: {
    delete(id) {
      MachineFamilyTypeDeleteAsync({ ID: id }).then(res => {
        if (res.Success) {
    ...
    handleDelete(item) {
      this.$confirm(this.$t('delete') + this.$t('affirm'), this.$t('hint'), {
        ...
        MachineFamilyTypeDeleteAsync(item).then(res => {
          if (res.Success) {
            this.handleSearch();
          }
        })
    ...
    getData() {
      ...
      this.loading = true
      MachineFamilyTypeFindWithPagerSearchAsync(params).then(res => {
