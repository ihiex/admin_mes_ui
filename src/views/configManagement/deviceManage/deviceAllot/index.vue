<template>
  <div v-loading="loading" class="page_container">
    <z-query ref="ZQuery" v-model.trim="form.Keywords" style="height: inherit;" :show-add="showAdd" :total="total"
      ...
      <el-button slot="button" v-hasPermi="['deviceAllot/BeachDelete']" type="danger" icon="el-icon-delete"
          ...
          <el-row :gutter="10">
            <el-col :xs="24" :sm="12" :md="8" :lg="8" :xl="8">
              <el-form-item :label="$t('PathName')">
                <el-select v-model="form.RouteIds" :placeholder="$t('pleaseSelect')" style="width: 100%;"
            ...
            <el-col :xs="24" :sm="12" :md="8" :lg="8" :xl="8">
              <el-form-item :label="$t('DevicePart')">
                <el-select v-model="form.MachinePartIds" :placeholder="$t('pleaseSelect')" style="width: 100%;"
                  :collapse-tags="true" :reserve-keyword="true" multiple filterable clearable>
                  <el-option v-for="item in partOptions" :key="item.ID" :label="item.PartNumber" :value="item.ID">
                    <div class="select_option">
                      <span>{{ item.PartNumber }}</span>
            ...
            <el-col :xs="24" :sm="12" :md="8" :lg="8" :xl="8">
              <el-form-item :label="$t('DeviceFamily')">
                <el-select v-model="form.MachineFamilyIds" :placeholder="$t('pleaseSelect')" style="width: 100%;"
                  :collapse-tags="true" :reserve-keyword="true" multiple filterable clearable>
      ...
      <div slot="table" style="height: 100%;">
        <el-table v-if="update" :data="tableData" style="width: 100%;" :default-expand-all="expandAll" height="100%"
          ...
          <el-table-column prop="RouteName" :label="$t('PathName')" sortable min-width="200"
            show-overflow-tooltip></el-table-column>
          ...
          <el-table-column prop="SN" :label="$t('DeviceSN')" sortable min-width="200"
            show-overflow-tooltip></el-table-column>
          <el-table-column prop="PartNumber" :label="$t('PartNumber')" sortable min-width="180" show-overflow-tooltip></el-table-column>
          <el-table-column prop="MachinePartNumber" :label="$t('DevicePart')" sortable min-width="200" show-overflow-tooltip></el-table-column>
          <el-table-column prop="MachineFamilyName" :label="$t('DeviceFamily')" sortable min-width="150" show-overflow-tooltip></el-table-column>
          <el-table-column prop="" :label="$t('operation')" width="100" fixed="right">
            <template slot-scope="props">
              <el-button v-hasPermi="['deviceAllot/Edit']" size="mini" type="primary" icon="el-icon-edit" circle
                @click="handleEdit(props.row)"></el-button>
              <el-button v-hasPermi="['deviceAllot/Delete']" size="mini" type="danger" icon="el-icon-delete" circle
                @click="handleDelete(props.row)"></el-button>
              <!-- <el-button v-hasPermi="['deviceAllot/Clone']" size="mini" icon="el-icon-copy-document" circle
...
<script>
import {
  MachineRouteDeleteAsync,
  MachineRouteFindWithPagerSearchAsync,
  getMachineFamily,
  getMachine,
} from '@/api/config.js'
...
export default {
  ...
  mounted() {
    ...
    let res = per.some(r => r === 'deviceAllot/Add')
    if (res) {
      this.showAdd = true
    } else {
      this.showAdd = false
  ...
  methods: {
    delete(id) {
      MachineRouteDeleteAsync({ ID: id }).then(res => {
        if (res.Success) {
    ...
    handleDelete(item) {
      this.$confirm(this.$t('delete') + this.$t('affirm'), this.$t('hint'), {
        ...
        MachineRouteDeleteAsync(item).then(res => {
          if (res.Success) {
            this.handleSearch();
          }
        })
    ...
    getData() {
      ...
      this.loading = true
      MachineRouteFindWithPagerSearchAsync(params).then(res => {
