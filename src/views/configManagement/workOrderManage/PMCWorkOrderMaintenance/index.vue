<template>
  <div v-loading="loading" class="page_container">
    <z-query ref="ZQuery" v-model.trim="form.Keywords" style="height: inherit;" :show-add="showAdd" :total="total"
      ...
      <el-button slot="button" v-hasPermi="['PMCWOMaintenance/BeachDelete']" type="danger" icon="el-icon-delete"
      ...
      <div slot="table" style="height: 100%;">
        <el-table v-if="update" :data="tableData" style="width: 100%;" border :default-expand-all="expandAll"
            ...
            <template slot-scope="props">
              <el-table :data="props.row.productionOrderDetails" style="width: 100%" size="mini" border :show-header="true">
                ...
                <el-table-column prop="" :label="$t('operation')" width="100">
                  <template slot-scope="scope">
                    <el-button v-hasPermi="['PMCWOMaintenance/EditChild']" size="mini" type="primary" icon="el-icon-edit"
                      circle @click="handleChildrenEdit(scope.row)"></el-button>
                    <el-button v-hasPermi="['PMCWOMaintenance/DeleteChild']" size="mini" type="danger"
                      icon="el-icon-delete" circle @click="handleChildrenDelete(scope.row)"></el-button>
                  </template>
                </el-table-column>
              </el-table>
              <!-- 分线 -->
              <el-table :data="props.row.productionOrderLines" style="width: 100%" size="mini" :show-header="true">
                ...
                <el-table-column prop="" :label="$t('operation')" width="100">
                  <template slot-scope="scope">
                    <el-button v-hasPermi="['PMCWOMaintenance/LineEdit']" size="mini" type="primary" icon="el-icon-edit"
                      circle @click="handleLineEdit(scope.row)"></el-button>
                    <el-button v-hasPermi="['PMCWOMaintenance/LineDelete']" size="mini" type="danger"
                      icon="el-icon-delete" circle @click="handleLineDelete(scope.row)"></el-button>
                  </template>
                </el-table-column>
              </el-table>
            </template>
          ...
          <el-table-column prop="" :label="$t('operation')" width="205" fixed="right">
            <template slot-scope="props">
              <el-button v-hasPermi="['PMCWOMaintenance/Edit']" size="mini" type="primary" icon="el-icon-edit" circle
                @click="handleEdit(props.row)"></el-button>
              <el-button v-hasPermi="['PMCWOMaintenance/Delete']" size="mini" type="danger" icon="el-icon-delete" circle
                @click="handleDelete(props.row)"></el-button>
              <el-button v-hasPermi="['PMCWOMaintenance/AddChild']" size="mini" icon="el-icon-plus" circle
                :title="$t('AddDetail')" @click="handleAddChildren(props.row)"></el-button>
              <el-button v-hasPermi="['PMCWOMaintenance/LineAdd']" size="mini" circle icon="el-icon-s-operation"
                :title="$t('AddLine')" @click="handleAddLine(props.row)"></el-button>
              <el-button v-hasPermi="['PMCWOMaintenance/Clone']" size="mini" icon="el-icon-copy-document" circle
...
import {
  ...
  WorkOrderMaintenanceFindWithPagerSearchPMCAsync,
  WorkOrderMaintenanceChildDeleteAsync,
  // WorkOrderAttributeFindWithAllPagerAsync,
  WorkOrderMaintenanceLineDeleteAsync
} from '@/api/config.js'
import { GetluProductionOrderStatus, GetmesPart, GetmesLine, GetluProductionOrderDetailDef_PMC } from '@/api/configCommon.js'
import mixin from '@/views/configManagement/mixin/index.js';
import PrimaryAddOrEdit from './bodyAddOrEdit.vue';
...
export default {
  ...
  mounted() {
    ...
    let res = per.some(r => r === 'PMCWOMaintenance/Add')
    if (res) {
      this.showAdd = true
    } else {
      this.showAdd = false
  ...
  methods: {
    ...
    getAttributeOptions() {
      GetluProductionOrderDetailDef_PMC().then(res => {
        if (res.Success) {
    ...
    getData() {
      ...
      this.loading = true
      WorkOrderMaintenanceFindWithPagerSearchPMCAsync(params).then(res => {
