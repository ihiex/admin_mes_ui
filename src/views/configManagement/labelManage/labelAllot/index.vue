<template>
  <div v-loading="loading" class="page_container">
    <z-query ref="ZQuery" v-model.trim="form.LikeQuery" style="height: inherit;" :show-add="showAdd" :total="total"
      ...
      <el-button slot="button" v-hasPermi="['labelAllot/BeachDelete']" type="danger" icon="el-icon-delete"
          ...
          <el-row :gutter="10">
            ...
            <el-col :xs="24" :sm="12" :md="8" :lg="8" :xl="8">
              <el-form-item :label="$t('Label')" prop="LabelID">
                <el-select v-model="form.LabelID" :placeholder="$t('pleaseSelect')" style="width: 100%;"
                  :collapse-tags="true" :reserve-keyword="true" multiple filterable clearable @focus="handleLabelFocus">
      ...
      <div slot="table" style="height: 100%;">
        <el-table v-if="update" :data="tableData" style="width: 100%;" border height="100%" :row-style="handleRowStyle"
          @selection-change="handleSelectionChange">
          <el-table-column type="selection" width="55"></el-table-column>
          <el-table-column prop="ID" label="ID" sortable width="80" :sort-method="handleSortMethod"
            show-overflow-tooltip></el-table-column>
          <el-table-column prop="Label" :label="$t('Label')" sortable min-width="240"
            show-overflow-tooltip></el-table-column>
          <el-table-column prop="Part" :label="$t('PartNumber')" sortable min-width="180"
            show-overflow-tooltip></el-table-column>
          <el-table-column prop="PartFamily" :label="$t('PartFamily')" sortable min-width="120"
            show-overflow-tooltip></el-table-column>
          <el-table-column prop="Line" :label="$t('Line')" sortable min-width="180"
            show-overflow-tooltip></el-table-column>
          <el-table-column prop="ProductionOrder" :label="$t('ProductionOrder')" sortable min-width="180"
            show-overflow-tooltip></el-table-column>
          <el-table-column prop="StationType" :label="$t('StationType')" sortable min-width="210"
            show-overflow-tooltip></el-table-column>
          <el-table-column prop="" :label="$t('operation')" width="130" fixed="right">
            <template slot-scope="props">
              <el-button v-hasPermi="['labelAllot/Edit']" size="mini" type="primary" icon="el-icon-edit" circle
                @click="handleEdit(props.row)"></el-button>
              <el-button v-hasPermi="['labelAllot/Delete']" size="mini" type="danger" icon="el-icon-delete" circle
                @click="handleDelete(props.row.ID)"></el-button>
              <el-button v-hasPermi="['labelAllot/Clone']" size="mini" icon="el-icon-copy-document" circle
...
<script>
import {
  LabelFormatAllotDelete,
  LabelFormatAllotFindWithPagerSearchAsync,
} from '@/api/config.js'
...
export default {
  ...
  mounted() {
    ...
    let res = per.some(r => r === 'labelAllot/Add')
    if (res) {
      this.showAdd = true
    } else {
      this.showAdd = false
  ...
  methods: {
    delete(id) {
      LabelFormatAllotDelete({ Id: id }).then(res => {
        if (res.Success) {
    ...
    getData() {
      ...
      this.loading = true
      LabelFormatAllotFindWithPagerSearchAsync(params).then(res => {
