<template>
  <div v-loading="loading" class="page_container">
    <z-query ref="ZQuery" v-model.trim="form.LikeQuery" style="height: inherit;" :show-add="showAdd" :total="total"
      ...
      <el-button slot="button" v-hasPermi="['employeeAccess/BeachDelete']" type="danger" icon="el-icon-delete"
      ...
      <div style="margin-top: 20px;">
        <el-form ref="form" :model="form" label-width="90px" @submit.native.prevent>
          <el-row :gutter="10">
            ...
            <el-col :xs="24" :sm="12" :md="8" :lg="8" :xl="8">
              <el-form-item :label="$t('status')" prop="StatusValue">
                <el-select v-model="form.StatusValue" :placeholder="$t('pleaseSelect')" style="width: 100%;" filterable
                  clearable>
      ...
      <div slot="table" style="height: 100%;">
        <el-table v-if="update" :data="tableData" style="width: 100%;" border height="100%" :row-style="handleRowStyle"
          ...
          <el-table-column prop="Employee" :label="$t('compellation')" sortable min-width="130"
            show-overflow-tooltip></el-table-column>
          <el-table-column prop="StationType" :label="$t('StationType')" sortable min-width="150"
            show-overflow-tooltip></el-table-column>
          ...
          <el-table-column prop="" :label="$t('operation')" width="130" fixed="right">
            <template slot-scope="props">
              <el-button v-hasPermi="['employeeAccess/Edit']" size="mini" type="primary" icon="el-icon-edit" circle
                @click="handleEdit(props.row)"></el-button>
              <el-button v-hasPermi="['employeeAccess/Delete']" size="mini" type="danger" icon="el-icon-delete" circle
                @click="handleDelete(props.row.ID)"></el-button>
              <el-button v-hasPermi="['employeeAccess/Clone']" size="mini" icon="el-icon-copy-document" circle
...
import {
  EmployeeDelete,
  EmployeeFindWithPagerSearchAsync,
} from '@/api/config.js'
...
export default {
  ...
  mounted() {
    ...
    let res = per.some(r => r === 'employeeAccess/Add')
    if (res) {
      this.showAdd = true
    } else {
      this.showAdd = false
  ...
  methods: {
    ...
    getData() {
      ...
      this.loading = true
      EmployeeFindWithPagerSearchAsync(params).then(res => {
