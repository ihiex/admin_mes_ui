<template>
  <div v-loading="loading" class="page_container">
    <z-query ref="ZQuery" v-model.trim="form.LikeQuery" style="height: inherit;" :show-add="showAdd" :total="total"
      ...
      <el-button slot="button" v-hasPermi="['SNType/BeachDelete']" type="danger" icon="el-icon-delete"
        ...
        <el-table v-if="update" :data="tableData" style="width: 100%;" height="100%" :row-style="handleRowStyle" border
          ...
          <el-table-column prop="" :label="$t('operation')" width="130" fixed="right">
            <template slot-scope="props">
              <el-button v-hasPermi="['SNType/Edit']" size="mini" type="primary" icon="el-icon-edit" circle
                @click="handleEdit(props.row)"></el-button>
              <el-button v-hasPermi="['SNType/Delete']" size="mini" type="danger" icon="el-icon-delete" circle
                @click="handleDelete(props.row.ID)"></el-button>
              <el-button v-hasPermi="['SNType/Clone']" size="mini" icon="el-icon-copy-document" circle
...
import {
  SNTypeDelete,
  SNTypeFindWithPagerSearchAsync,
} from '@/api/config.js'
...
export default {
  ...
  mounted() {
    ...
    let res = per.some(r => r === 'SNType/Add')
    if (res) {
      this.showAdd = true
    } else {
      this.showAdd = false
  ...
  methods: {
    ...
    getData() {
      ...
      this.loading = true
      SNTypeFindWithPagerSearchAsync(params).then(res => {
