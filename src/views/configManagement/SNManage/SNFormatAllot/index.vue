<template>
  <div v-loading="loading" class="page_container">
    <z-query ref="ZQuery" v-model.trim="form.LikeQuery" style="height: inherit;" :show-add="showAdd" :total="total"
      ...
      <el-button slot="button" v-hasPermi="['SNFormatAllot/BeachDelete']" type="danger" icon="el-icon-delete"
      ...
      <div slot="table" style="height: 100%;">
        <el-table v-if="update" :data="tableData" style="width: 100%;" border height="100%" :row-style="handleRowStyle"
          ...
          <el-table-column prop="SNFormat" :label="$t('SNFormat')" sortable min-width="240"
            show-overflow-tooltip></el-table-column>
          ...
          <el-table-column prop="" :label="$t('operation')" width="130" fixed="right">
            <template slot-scope="props">
              <el-button v-hasPermi="['SNFormatAllot/Edit']" size="mini" type="primary" icon="el-icon-edit" circle
                @click="handleEdit(props.row)"></el-button>
              <el-button v-hasPermi="['SNFormatAllot/Delete']" size="mini" type="danger" icon="el-icon-delete" circle
                @click="handleDelete(props.row.ID)"></el-button>
              <el-button v-hasPermi="['SNFormatAllot/Clone']" size="mini" icon="el-icon-copy-document" circle
...
import {
  SNFormatAllotDelete,
  SNFormatAllotFindWithPagerSearchAsync,
} from '@/api/config.js'
...
export default {
  ...
  mounted() {
    ...
    let res = per.some(r => r === 'SNFormatAllot/Add')
    if (res) {
      this.showAdd = true
    } else {
      this.showAdd = false
  ...
  methods: {
    delete(id) {
      SNFormatAllotDelete({ Id: id }).then(res => {
        if (res.Success) {
    ...
    getData() {
      ...
      this.loading = true
      SNFormatAllotFindWithPagerSearchAsync(params).then(res => {
