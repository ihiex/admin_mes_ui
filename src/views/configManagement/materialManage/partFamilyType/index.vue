<template>
  <div v-loading="loading" class="page_container">
    <z-query ref="ZQuery" v-model.trim="form.Keywords" style="height: inherit;" :show-add="showAdd" :total="total"
      ...
      <el-button slot="button" v-hasPermi="['partFamilyType/BeachDelete']" type="danger" icon="el-icon-delete"
        ...
        <el-table v-if="update" :data="tableData" style="width: 100%;" :default-expand-all="expandAll" height="100%" border
          ...
          <el-table-column type="expand">
            <template slot-scope="props">
              <!-- <el-row v-for="item in props.row.PartFamilyTypeDetails" :key="item.ID" :gutter="10"
                ...
                <el-col :span="4">
                  <el-button v-hasPermi="['partFamilyType/EditChild']" size="mini" type="primary" icon="el-icon-edit"
                    circle @click="handleChildrenEdit(item)"></el-button>
                  <el-button v-hasPermi="['partFamilyType/DeleteChild']" size="mini" type="danger" icon="el-icon-delete"
                    circle @click="handleChildrenDelete(item)"></el-button>
                </el-col>
              </el-row> -->
              <el-table :data="props.row.PartFamilyTypeDetails" style="width: 100%" :show-header="false">
          ...
          <el-table-column prop="Name" :label="$t('PartFamilyType')" sortable min-width="150"
            show-overflow-tooltip></el-table-column>
          ...
          <el-table-column prop="" :label="$t('operation')" width="170" fixed="right">
            <template slot-scope="props">
              <el-button v-hasPermi="['partFamilyType/Edit']" size="mini" type="primary" icon="el-icon-edit" circle
                @click="handleEdit(props.row)"></el-button>
              <el-button v-hasPermi="['partFamilyType/Delete']" size="mini" type="danger" icon="el-icon-delete" circle
                @click="handleDelete(props.row)"></el-button>
              <el-button v-hasPermi="['partFamilyType/AddChild']" size="mini" icon="el-icon-plus" circle
                @click="handleAddChildren(props.row.ID)"></el-button>
              <el-button v-hasPermi="['partFamilyType/Clone']" size="mini" icon="el-icon-copy-document" circle
...
<script>
import {
  PartFamilyTypeDeleteAsync,
  PartFamilyTypeFindWithPagerSearchAsync,
  PartFamilyTypeChildDeleteAsync,
...
import PrimaryAddOrEdit from './primaryAddOrEdit';
...
export default {
  ...
  mounted() {
    ...
    let res = per.some(r => r === 'partFamilyType/Add')
    if (res) {
      this.showAdd = true
    } else {
      this.showAdd = false
  ...
  methods: {
    delete(id) {
      PartFamilyTypeDeleteAsync({ ID: id }).then(res => {
        if (res.Success) {
    ...
    handleChildrenDelete(item) {
      this.$confirm(this.$t('delete') + this.$t('affirm'), this.$t('hint'), {
        ...
        PartFamilyTypeChildDeleteAsync(item).then(res => {
          if (res.Success) {
            this.getData();
          }
        })
    ...
    handleDelete(item) {
      this.$confirm(this.$t('delete') + this.$t('affirm'), this.$t('hint'), {
        ...
        PartFamilyTypeDeleteAsync(item).then(res => {
          if (res.Success) {
            this.handleSearch();
          }
        })
    ...
    getData() {
      this.loading = true
      PartFamilyTypeFindWithPagerSearchAsync(this.form).then(res => {
