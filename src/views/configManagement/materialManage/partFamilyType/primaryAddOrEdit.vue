  ...
  <el-dialog :title="title" :visible.sync="dialogVisible" width="400" :close-on-click-modal="false"
    ...
    <div v-loading="loading">
      <el-form ref="form" :model="form" :rules="rules" label-width="120px">
        <el-form-item :label="$t('PartFamilyType')" prop="Name">
...
<script>
import {
  PartFamilyTypeInsertAsync,
  PartFamilyTypeUpdateAsync,
  PartFamilyTypeCloneAsync
} from '@/api/config.js'
    ...
    confirm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.loading = true
          if (this.flag === 'Edit') {
            PartFamilyTypeUpdateAsync(this.form).then(res => {
              if (res.Success) {
            ...
            PartFamilyTypeInsertAsync(this.form).then(res => {
              if (res.Success) {
                this.$emit('confirm', this.flag)
              }
              this.loading = false
            ...
            PartFamilyTypeCloneAsync(this.form).then(res => {
              if (res.Success) {
                this.$emit('confirm', this.flag)
              }
              this.loading = false
