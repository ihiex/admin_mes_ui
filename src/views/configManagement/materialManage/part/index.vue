<template>
  <div v-loading="loading" class="page_container">
    <z-query ref="ZQuery" v-model.trim="form.Keywords" style="height: inherit;" :show-add="showAdd" :total="total"
      ...
      <el-button slot="button" v-hasPermi="['part/BeachDelete']" type="danger" icon="el-icon-delete"
        ...
        <el-table v-if="update" :data="tableData" style="width: 100%;" :default-expand-all="expandAll" height="100%" border
          ...
          <el-table-column type="expand">
            <template slot-scope="props">
              <!--  <el-row v-for="item in props.row.PartDetails" :key="item.ID" :gutter="10"
                ...
                <el-col :span="4">
                  <el-button v-hasPermi="['part/EditChild']" size="mini" type="primary" icon="el-icon-edit" circle
                    @click="handleChildrenEdit(item)"></el-button>
                  <el-button v-hasPermi="['part/DeleteChild']" size="mini" type="danger" icon="el-icon-delete" circle
                    @click="handleChildrenDelete(item)"></el-button>
                </el-col>
              </el-row> -->
              <el-table :data="props.row.PartDetails" style="width: 100%" :show-header="false">
          ...
          <el-table-column prop="PartFamilyName" :label="$t('PartFamily')" sortable min-width="120"
            show-overflow-tooltip></el-table-column>
          <el-table-column prop="PartNumber" :label="$t('PartNumber')" sortable min-width="130"
            show-overflow-tooltip></el-table-column>
          <el-table-column prop="Description" :label="$t('describe')" sortable min-width="180"
            show-overflow-tooltip></el-table-column>
          <el-table-column prop="StatusDesc" :label="$t('status')" sortable min-width="100"
            show-overflow-tooltip></el-table-column>
          <el-table-column prop="IsUnit" :label="$t('IsUnit')" sortable min-width="100" show-overflow-tooltip>
            <template slot-scope="scope">
              {{ scope.row.IsUnit ? 'true' : 'false' }}
            </template>
          </el-table-column>
          <el-table-column prop="" :label="$t('operation')" width="170" fixed="right" show-overflow-tooltip>
            <template slot-scope="props">
              <el-button v-hasPermi="['part/Edit']" size="mini" type="primary" icon="el-icon-edit" circle
                @click="handleEdit(props.row)"></el-button>
              <el-button v-hasPermi="['part/Delete']" size="mini" type="danger" icon="el-icon-delete" circle
                @click="handleDelete(props.row)"></el-button>
              <el-button v-hasPermi="['part/AddChild']" size="mini" icon="el-icon-plus" circle
                @click="handleAddChildren(props.row.ID)"></el-button>
              <el-button v-hasPermi="['part/Clone']" size="mini" icon="el-icon-copy-document" circle
...
import {
  PartDeleteAsync,
  PartFindWithPagerSearchAsync,
  PartChildDeleteAsync,
...
export default {
  ...
  methods: {
    delete(id) {
      PartDeleteAsync({ ID: id }).then(res => {
        if (res.Success) {
    ...
    handleChildrenDelete(item) {
      this.$confirm(this.$t('delete') + this.$t('affirm'), this.$t('hint'), {
        ...
        PartChildDeleteAsync(item).then(res => {
          if (res.Success) {
            this.getData();
          }
        })
    ...
    getData() {
      ...
      this.loading = true
      PartFindWithPagerSearchAsync(params).then(res => {
