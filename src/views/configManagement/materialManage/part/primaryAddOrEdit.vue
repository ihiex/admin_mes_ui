    ...
    <div v-loading="loading">
      <el-form ref="form" :model="form" :rules="rules" label-width="120px">
        ...
        <el-form-item :label="$t('PartNumber')" prop="PartNumber">
          <el-input v-model="form.PartNumber" :placeholder="$t('pleaseInput')" clearable></el-input>
        </el-form-item>
        ...
        <el-form-item :label="$t('IsUnit')" prop="IsUnit">
          <el-select v-model="form.IsUnit" :placeholder="$t('pleaseSelect')" style="width: 100%;" clearable filterable>
            <el-option v-for="item in $store.getters['common/IsUnit']" :key="item.value" :label="item.label"
              :value="item.value">
            </el-option>
          </el-select>
        </el-form-item>
    ...
    confirm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.loading = true
          if (this.flag === 'Edit') {
            PartUpdateAsync(this.form).then(res => {
              if (res.Success) {
            ...
            PartInsertAsync(this.form).then(res => {
              if (res.Success) {
                this.$emit('confirm', this.flag)
              }
              this.loading = false
            ...
            PartCloneAsync(this.form).then(res => {
              if (res.Success) {
                this.$emit('confirm', this.flag)
              }
              this.loading = false
