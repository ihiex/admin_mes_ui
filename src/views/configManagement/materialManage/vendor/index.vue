<template>
  <div v-loading="loading" class="page_container">
    <z-query ref="ZQuery" v-model.trim="form.Keywords" style="height: inherit;" :show-add="showAdd" :total="total"
      ...
      <el-button slot="button" v-hasPermi="['vendor/BeachDelete']" type="danger" icon="el-icon-delete"
      ...
      <div slot="table" style="height: 100%;">
        <el-table v-if="update" :data="tableData" style="width: 100%;" :default-expand-all="expandAll" height="100%" border
          ...
          <el-table-column prop="ID" label="ID" sortable width="80" :sort-method="handleSortMethod" show-overflow-tooltip></el-table-column>
          <el-table-column prop="Code" :label="$t('VendorCode')" sortable min-width="140" show-overflow-tooltip></el-table-column>
          <el-table-column prop="Description" :label="$t('describe')" sortable min-width="180" show-overflow-tooltip></el-table-column>
          <el-table-column prop="PartName" :label="$t('PartNumber')" sortable min-width="130" show-overflow-tooltip></el-table-column>
          <el-table-column prop="" :label="$t('operation')" width="100" fixed="right">
            <template slot-scope="props">
              <el-button v-hasPermi="['vendor/Edit']" size="mini" type="primary" icon="el-icon-edit" circle
                @click="handleEdit(props.row)"></el-button>
              <el-button v-hasPermi="['vendor/Delete']" size="mini" type="danger" icon="el-icon-delete" circle
                @click="handleDelete(props.row)"></el-button>
              <!-- <el-button v-hasPermi="['vendor/Clone']" size="mini" icon="el-icon-copy-document" circle
...
import {
  VendorDeleteAsync,
  VendorFindWithPagerSearchAsync,
...
export default {
  ...
  mounted() {
    ...
    let res = per.some(r => r === 'vendor/Add')
    if (res) {
      this.showAdd = true
    } else {
      this.showAdd = false
  ...
  methods: {
    delete(id) {
      VendorDeleteAsync({ ID: id }).then(res => {
        if (res.Success) {
    ...
    handleDelete(item) {
      this.$confirm(this.$t('delete') + this.$t('affirm'), this.$t('hint'), {
        ...
        VendorDeleteAsync(item).then(res => {
          if (res.Success) {
            this.handleSearch();
          }
        })
    ...
    getData() {
      ...
      this.loading = true
      VendorFindWithPagerSearchAsync(params).then(res => {
