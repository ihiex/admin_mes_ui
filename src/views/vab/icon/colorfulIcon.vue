...
<script>
  ...
  import iconData from './remixIcon'
  ...
  export default {
    ...
    methods: {
      ...
      async fetchData() {
        // let { data, totalCount } = await getIconList(this.queryForm)
        this.queryIcon = iconData
        this.allIcon = data
        this.total = Object.keys(iconData).length
      },
      handleCopyIcon(index, event) {
        //let copyText = `<vab-colorful-icon icon-class="https://cdn.jsdelivr.net/gh/chuzhixin/zx-colorful-icon@master/${this.queryIcon[index]}.svg" />`;
        let copyText = `<vab-colorful-icon icon-class="${this.queryIcon[index]}" />`
