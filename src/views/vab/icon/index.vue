...
export default {
  ...
  methods: {
    ...
    queryData() {
      ...
      this.queryIcon = this.allIcon.filter((res) =>
        res.includes(this.queryForm.title)
      )
    },
    async fetchData() {
      // let { data, totalCount } = await getIconList(this.queryForm)
      let data = iconData
      this.queryIcon = data //當前顯示的
      this.allIcon = data //緩存所有的
      this.total = Object.keys(iconData).length
    },
    handleCopyIcon(index, event) {
      let copyText = `<vab-icon :icon="['fas', '${this.queryIcon[index]}']"></vab-icon>`
...
:v-deep .layout-container-horizontal {
