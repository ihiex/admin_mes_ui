    ...
    <el-table :data="tableData" border style="width: 100%;" height="calc(100% - 0px)" size="mini"
      ...
      <el-table-column v-for="(item, key) in tableHead" :key="key" :label="key" :min-width="'200'" align='center'>
        <el-table-column v-for="res in item" :key="res" :prop="res"
          :label="res === 'StationType' ? 'Station' : res.split(/\s/g).pop()" align='center' min-width="45">
          <template slot-scope="scope">
            <span style="display: inline-block; height: 100%; width: 100%;"
              :style="{ 'background-color': handleReturnColor(scope.row, res), 'border-left': handleReturnBorderColor(scope.row, res) }">{{
                scope.row[res] }}</span>
          </template>
        </el-table-column>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleReturnBorderColor(item, key) {
      // console.log(item)
      // console.log(key)
    ...
    objectSpanMethod({ row, column, rowIndex, columnIndex }) {
      if (columnIndex === 0) {
        if (row.type === 'Input') { //需删除多余的数据配合
          return {
            rowspan: row.IsRPTYield === 0 ? 2 : 5,
            colspan: 1
          };
        }
        /* if (rowIndex % 5 === 0) {
    ...
    show(data, levelValue = 3, shift = [], dateTime = new Date().format('yyyy-MM-dd hh:mm:ss')) {
      ...
      2: 只显示input, output
      3：input, output, ng
      4: input, output, ng, script
      5：5个都显示
      */
      Object.keys(dataList).forEach(key => {
        for (let i = 0; i < dataList[key].length; i++) {
          if (dataList[key][i].IsRPTYield === 0 && (dataList[key][i].type === 'NG' || dataList[key][i].type === 'SCRAP' || dataList[key][i].type === 'Yield')) {
