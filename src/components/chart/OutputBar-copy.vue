              ...
              <el-table-column v-for="(h, i) in tableHead" :key="i" :prop="h"
                ...
                <template slot-scope="scope">
                  ...
                  <span v-if="!isNaN(scope.row[h])" style="cursor: pointer;" @click="handleGoToRawData(scope.row, h)">
                    {{ scope.row[h] }}
                    <!-- <span
                      v-if="((h == 'total' || h.includes('WH') || !isNaN(parseInt(h))) && ((scope.row.IsRPTYield == 1 && scope.row.type == 'Input') || scope.row.type == 'Output')) && isShowRow">
                      <el-progress style="position: relative; top: 3px" :width="20" type="circle" :stroke-width="3"
                        :percentage="returnCompleteRateV1(scope.row, h)" :show-text="false"
                        :title="returnCompleteRate(scope.row, h) + '%'"></el-progress> -->
                    <!-- {{ returnCompleteRate(scope.row, h) }} -->
                    <!-- style="display: inline-block; position: relative; top: 3px; height: 14px; width: 14px;border: 2px solid #ccc; border-radius: 100%;" -->
                    <!-- <div class="complete_rate" ></div> -->
                    <!-- </span> -->
                  </span>
                  <span v-else>
                    {{ scope.row[h] }}
                    <span
                      v-if="(h == 'total' || h.includes('WH') || !isNaN(parseInt(h))) && scope.row.type == 'completeRate'" style="padding-left: 5px;">
                      <el-progress style="position: relative; top: 3px" :width="16" type="circle" :stroke-width="3"
                        :percentage="returnCompleteRateV2(scope.row[h])" :show-text="false" :title="scope.row[h]"></el-progress>
                    </span>
                  </span>
                </template>
              </el-table-column>
