          ...
          <div class="table_content" :class="tableAutoClass + index">
            ...
            <el-table :data="item" border height="100%" size="mini" :span-method="objectSpanMethod"
              ...
              <el-table-column v-for="(h, i) in tableHead" :key="i" :prop="h"
                :label="h === 'UPHTarget' ? 'UPH' : h.split(/\s/g).pop()" :min-width="h == 'UPHTarget' ? '60' : '110'"
                ...
                <template slot-scope="scope">
                  ...
                  <span v-else :title="scope.row['UPHTargetByHour/' + h]?scope.row['UPHTargetByHour/' + h]:scope.row['UPHTarget']">
                    {{ scope.row[h] }}
                    <span
                      v-if="(h == 'total' || h.includes('WH') || !isNaN(parseInt(h))) && scope.row.type == 'completeRate'"
                      style="padding-left: 5px;">
                      <el-progress v-if="scope.row[h]" style="position: relative; top: 3px" :width="16" type="circle"
                        :stroke-width="3" :percentage="returnCompleteRateV2(scope.row[h])" :show-text="false">
                      </el-progress>
                    </span>
                  </span>
                </template>
            ...
            <div class="target_input">
              <el-dropdown>
                ...
                <el-dropdown-menu slot="dropdown">
                  <div style="display: inline-block; margin: 0 10px">target:</div>
                  <el-input v-model="target" type="number" style="width: 150px" @input="handleTargetInput"></el-input>
                  <div style="margin: 10px;">
                    <el-switch v-model="isShowRow" :active-text="$t('display')" :inactive-text="$t('hide')"
...
export default {
  ...
  data() {
    ...
    let completeRate = localStorage.getItem('OutputBarCompleteRate')
    return {
      chartContext: {},
      carouselItem: [],
      dateTime: '',
      ...
      time1: null,
      time2: null,
      isShowRow: completeRate ? JSON.parse(completeRate) : false,
      cacheObj: {},
      cum: 0
    }
  },
  ...
  methods: {
    returnCompleteRateV1(item, key) {
      ...
      if (key == 'total') {
        try {
          c = parseFloat(((item.sumOutput / item.sumTarget) * 100).toFixed(1))
        } catch {
          c = 0
        }
      } else {
        try {
          c = parseFloat(((item[key] / item.UPHTarget) * 100).toFixed(1))
        } catch {
          c = 0
        }
      }
    ...
    returnCompleteRateV2(item) {
      let res = parseFloat(item)
      return isNaN(res) ? 0 : res > 100 ? 100 : res
    },
    returnCompleteRate(item, key) {
      if (!item[key]) {
        return 0
      }
      let c = 0
    ...
    handleSwitchChange(val) {
      ...
      localStorage.setItem('OutputBarCompleteRate', val)
    },
    ...
    handleSourceDataExport() {
      let srcData = this.cacheObj?.data?.[2]
      if (!srcData) {
        this.$message.warning('Data exception')
        return false
      }

      srcData = JSON.parse(JSON.stringify(srcData))
      ...
      downloadFile(
        ...
        `Output_Source_Data_${new Date().format('yyyy/MM/dd hh:mm:ss')}.xlsx`,
      )
      return false
    },
    getRowspan(val) {
          ...
          for (let i = 0; i < values.length; i++) {
            ...
            if ((values[i + 1] && values[i].StationType != values[i + 1].StationType) || i == values.length - 1) {
              ...
              Object.keys(cacheValues).forEach(key => {
                if (key == 'total') {
                  obj[key] = ((cacheValues.sumOutput / (cacheValues.sumUPHTargetByHour ? cacheValues.sumUPHTargetByHour : cacheValues.sumTarget ? cacheValues.sumTarget : 1)) * 100).toFixed(1) + '%';
                } else if (key.includes('WK') || !isNaN(parseInt(key))) {
                  let v = cacheValues['UPHTargetByHour/' + key]
                  obj[key] = ((cacheValues[key] / (v ? v : cacheValues.UPHTarget ? cacheValues.UPHTarget : 1)) * 100).toFixed(1) + '%'
                } else {
    ...
    */
    ...
    HandleFormatData(data = [], levelValue = 3, type = 'Output') {
      ...
      data[2].forEach(res => {
        ...
        listObj[key]['UPHTargetByHour'] = res.UPHTargetByHour ? res.UPHTargetByHour : 0;
        listObj[key]['YieldTarget'] = res.YieldTarget ? res.YieldTarget : null;
      })
      ...
      Object.values(listObj).forEach(res => {
        ...
        if (tItem[key]) {
          ...
          tItem[key]['UPHTargetByHour/' + res.CDate] = res.UPHTargetByHour //? res.UPHTargetByHour : null;
          tItem[key]['sumUPHTargetByHour'] += res.UPHTargetByHour;
        } else {
          tItem[key] = {};
          ...
          tItem[key]['YieldTarget'] = res.YieldTarget // ? res.YieldTarget : null;
          tItem[key]['UPHTargetByHour/' + res.CDate] = res.UPHTargetByHour //? res.UPHTargetByHour : null;
          tItem[key]['sumUPHTargetByHour'] = res.UPHTargetByHour;
        }
      })
                ...
                data: [
                  {
                    ...
                    name: 'target',
                    yAxis: item[0].YieldTarget ? item[0].YieldTarget : target,
                  },
                ],
              },
            }
